...
type BuyOrder struct {
	...
	AmountUnfilled  float64          `json:"amountUnfilled,string"` // Only for ImmediateOrCancel buy
}
...
func (client *TradingClient) BuyFillOrKill(currencyPair string, rate, amount float64) (*BuyOrder, error) {
	return client.buy(currencyPair, rate, amount, "fillOrKill")
}

func (client *TradingClient) BuyImmediateOrCancel(currencyPair string, rate, amount float64) (*BuyOrder, error) {
	return client.buy(currencyPair, rate, amount, "immediateOrCancel")
}

func (client *TradingClient) BuyPostOnly(currencyPair string, rate, amount float64) (*BuyOrder, error) {
	return client.buy(currencyPair, rate, amount, "postOnly")
}

func (client *TradingClient) Buy(currencyPair string, rate, amount float64) (*BuyOrder, error) {
	return client.buy(currencyPair, rate, amount, "")
}

// Poloniex trading API implementation of buy command.
//
// API Doc:
...
//    ], ...
//  }
func (client *TradingClient) buy(currencyPair string, rate, amount float64, option string) (*BuyOrder, error) {

	postParameters := url.Values{}
	postParameters.Add("command", "buy")
